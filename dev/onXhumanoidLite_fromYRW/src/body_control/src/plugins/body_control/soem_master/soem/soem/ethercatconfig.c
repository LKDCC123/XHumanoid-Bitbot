...
extern void ecat_log_info(const char* message);
...
static void ecx_set_slaves_to_default(ecx_contextt *context)
{
   ...
   ecx_BWR(context->port, 0x0000, ECT_REG_FRXERR      , 4         , &zbuf, EC_TIMEOUTRET3);
   ecx_BWR(context->port, 0x0000, ECT_REG_EPUECNT     , 1         , &zbuf, EC_TIMEOUTRET3);
...
extern unsigned int g_uiRcvTime;


void ec_Get_Diagnosis_Info()
{
	ecx_contextt *context = &ecx_context;
	char strLog[1024] = {0};
	unsigned char ucEPUECNT = 0;
	unsigned int uiSlave = 1, uiPortForwardErrInfo = 0, uiPortLinkLostInfo = 0;
	unsigned long long ullPortERRInfo = 0;
	unsigned short usState = 0, usErrCode = 0;
	int iRet = 0, iRetWhole = 0;
	for (uiSlave = 1; uiSlave <= *(context->slavecount); uiSlave++)
	{
		ucEPUECNT = 0;
		uiPortForwardErrInfo = 0, uiPortLinkLostInfo = 0;
		ullPortERRInfo = 0;
		usState = 0, usErrCode = 0;
		
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_ALSTAT, sizeof(usState), &usState, 200);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x130 fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_ALSTATCODE, sizeof(usErrCode), &usErrCode, 200);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x134 fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_RXERR, sizeof(ullPortERRInfo), &ullPortERRInfo, 400);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x300 fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_FRXERR, sizeof(uiPortForwardErrInfo), &uiPortForwardErrInfo, 200);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x308 fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_EPUECNT, sizeof(ucEPUECNT), &ucEPUECNT, 200);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x30c fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_LLCNT, sizeof(uiPortLinkLostInfo), &uiPortLinkLostInfo, 200);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x310 fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
		//for debug
		unsigned long long ullSM2Reg = 0;
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_SM2, sizeof(ullSM2Reg), &ullSM2Reg, 800);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x810 fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
/*
		unsigned int uiRcvTime = 0;
		iRet = ecx_FPRD(context->port, context->slavelist[uiSlave].configadr, ECT_REG_DCTIME0, sizeof(uiRcvTime), &uiRcvTime, 200);
		if(0 >= iRet)
		{
			sprintf(strLog, "slave %d get 0x900 fail return wkc = %d", uiSlave, iRet);
			ecat_log_info(strLog);
		}
		*/
/*
		sprintf(strLog, "slave %d diagnosis 0x130:0x%x 0x134:0x%x 0x300:0x%x 0x308:0x%x 0x30c:0x%x 0x310:0x%x", uiSlave,
			usState, usErrCode, ullPortERRInfo, uiPortForwardErrInfo, ucEPUECNT, uiPortLinkLostInfo);
*/

		//for debug
		sprintf(strLog, "slave %d diagnosis 0x130:0x%x 0x134:0x%x 0x300:0x%x 0x308:0x%x 0x30c:0x%x 0x310:0x%x 0x810:0x%llx", uiSlave,
			usState, usErrCode, ullPortERRInfo, uiPortForwardErrInfo, ucEPUECNT, uiPortLinkLostInfo, ullSM2Reg);
			/*
		sprintf(strLog, "slave %d diagnosis 0x130:0x%x 0x134:0x%x 0x300:0x%x 0x308:0x%x 0x30c:0x%x 0x310:0x%x 0x810:0x%llx g_uiRcvTime:0x%x uiRcvTime:0x%x",
		uiSlave, usState, usErrCode, ullPortERRInfo, uiPortForwardErrInfo, ucEPUECNT, uiPortLinkLostInfo, ullSM2Reg, g_uiRcvTime, uiRcvTime);
				*/
		ecat_log_info(strLog);
