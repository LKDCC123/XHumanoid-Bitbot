...
static OSAL_THREAD_FUNC ecatcheck(void *ptr)
{
    (void)ptr;
    int slave = 0;
	unsigned int uiCheckPeriodByUs = 50000, uiPrintDiagnosisPeriodByUs = 3000000;
	unsigned int uiDiagnosisPeriodByCheckPeriod = uiPrintDiagnosisPeriodByUs / uiCheckPeriodByUs;//诊断信息每隔多少次检查循环打印一次
	unsigned int uiDiagnosisPeriodByCheckPeriodCount = 0;//给诊断信息使用的检查循环次数累计
    while (1)
    {
		if(inOP && ++uiDiagnosisPeriodByCheckPeriodCount >= uiDiagnosisPeriodByCheckPeriod)
		{
			uiDiagnosisPeriodByCheckPeriodCount = 0;
			ec_Get_Diagnosis_Info();
		}		
        // count errors
...
void SoemMaster::Run()
{
    ...
    while (running) {
        ...
        if (wkc < expectedWKC)
        {
            LOG(ERROR) << "[EtherCAT Error] Dropped packet (Bad WKC!), expectedWKC " << expectedWKC << "actual wkc "<< wkc;
			ec_Get_Diagnosis_Info();
            wkc_err_count++;
        }
        else
...
// 输出信息
void ecat_log_info(const char* message) {
    LOG(INFO) << message;
}
// 输出警告
void ecat_log_warning(const char* message) {
    LOG(WARNING) << message;
}
// 输出错误
void ecat_log_error(const char* message) {
    LOG(ERROR) << message;
}
