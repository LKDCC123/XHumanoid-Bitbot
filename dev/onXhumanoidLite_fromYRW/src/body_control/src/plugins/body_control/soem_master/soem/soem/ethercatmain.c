         ...
         if(context->grouplist[group].Ibytes)
         {
            ...
            do
            {
				...
				if(first)
               {
                  context->DCl = sublength;
                  /* FPRMW in second datagram */
                  context->DCtO = ecx_adddatagram(context->port, &(context->port->txbuf[idx]), EC_CMD_FRMW, idx, TRUE,
                                           context->slavelist[context->grouplist[group].DCnext].configadr,
                                           ECT_REG_DCSYSTIME, sizeof(int64), context->DCtime);
                  first = FALSE;
               }
				
               {
                  ...
                  ecx_adddatagram(context->port, &(context->port->txbuf[idx]), EC_CMD_FPWR, idx, TRUE,
                                           context->slavelist[1].configadr,
                                           ECT_REG_DCTIME0, sizeof(g_uiRcvTime ), &g_uiRcvTime );
				  ecx_adddatagram(context->port, &(context->port->txbuf[idx]), EC_CMD_FPRD, idx, FALSE,
                                           context->slavelist[1].configadr,
                                           ECT_REG_DCTIME0, sizeof(g_uiRcvTime ), &g_uiRcvTime );
               }
#endif
